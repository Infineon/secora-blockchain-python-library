import logging
from blocksec2go.comm.scp03 import SCP03
...
class Apdu:
    ...
    """
    def __init__(self, header, data = b'', le = None):
        if len(header) != 4:
            raise ValueError('invalid header length: ' + str(header) + ' (' + str(len(header)) + ')')
        if len(data) > 255 or (le is not None and le > 256):
            raise RuntimeError('extended length APDU support not yet implemented')

        sec_channel = SCP03()
        (self.header, self.data) = sec_channel.wrapApdu(header, data)
